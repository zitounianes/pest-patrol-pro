    ...
    <section id="articulos" className="py-28 relative" ref={ref}>
      <div className="container mx-auto px-4 md:px-8">
        <div className="text-center mb-16 reveal">
          <span className="text-[hsl(var(--warm))] text-sm font-semibold uppercase tracking-[0.2em] mb-4 block">Blog</span>
        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto reveal reveal-delay-1">
            ...
            <Link key={i} to={article.link} className="group bg-card border border-border rounded-3xl overflow-hidden shadow-card hover:shadow-cinematic hover:-translate-y-1 transition-all duration-300 block">
              <div className="p-7">
                <div className="flex items-center justify-between mb-5">
                  ...
                  <span className="text-xs font-semibold uppercase tracking-wider text-[hsl(var(--warm))] bg-[hsl(var(--warm),0.08)] px-3 py-1 rounded-full">{article.tag}</span>
                </div>
                <h3 className="font-body font-semibold text-lg mb-3 group-hover:text-primary transition-colors leading-snug">{article.title}</h3>
                <p className="text-[15px] text-muted-foreground leading-relaxed mb-5">{article.desc}</p>
